...
export default function NearbyPeopleModal({ allUsers, userLoc, currentUser, followedEmails = [], onClose, onHighlight }) {
  ...
  const positioned = useMemo(() => {
    return nearby.map((p) => {
      ...
      const normDist = Math.max(0.15, p._dist / maxDist);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative mx-auto mt-2" style={{ width: "min(90vw, 340px)", aspectRatio: "1" }} ref={radarRef}>
          <svg className="absolute inset-0 w-full h-full" viewBox={`0 0 ${radarSize} ${radarSize}`}>
            {[0.25, 0.5, 0.75, 1].map(scale => (
              <circle key={scale} cx={r} cy={r} r={r * scale * 0.96}
          ...
          {radarPeople.map(p => (
            ...
            >
              <div
                className="w-12 h-12 rounded-2xl overflow-hidden flex items-center justify-center font-bold text-white text-sm"
                style={{
                  background: p.avatar_url ? "transparent" : `linear-gradient(135deg, ${getColor(p.user_email)}, ${getColor(p.user_email)}aa)`,
                  border: "2px solid rgba(0,210,120,0.5)"
                }}
              >
                {p.avatar_url
        ...
        {/* Mini profile popup for selected person */}
        {selectedPerson && (
          <div className="mx-4 mt-3 rounded-2xl p-3 flex items-center gap-3"
            ...
            <button onClick={() => setSelectedPerson(null)}
              ...
              style={{ backgroundColor: "rgba(255,255,255,0.1)", minWidth: 24, minHeight: 24 }}>
              <X className="w-3 h-3 text-white" />
            </button>
          </div>
        )}

        {/* Count + more people row */}
