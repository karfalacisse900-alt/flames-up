...
export default function SavePostModal({ post, user, onClose }) {
  ...
  /*
    FIX — Save to Collection overlay bug:
    The modal was rendered inside CommunityPostCard's DOM tree, which sits inside
    a scrollable feed. Any ancestor with `transform`, `overflow:hidden`, or a lower
    `z-index` stacking context would clip or hide the overlay.

    Solution: Use React.createPortal() to mount the modal directly into document.body,
    completely outside the card's DOM hierarchy. This guarantees the backdrop and
    sheet always appear on top of everything else regardless of ancestor styles.
  */
  const modalContent = (
    <AnimatePresence>
      <motion.div
        ...
        style={{
          ...
          zIndex: 9999,
          // Isolate stacking context so nothing below can bleed through
          isolation: "isolate",
        }}
        onClick={onClose}
      >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          ...
                          style={{ color: saved ? "var(--accent-primary)" : "var(--text-primary)" }}
                        >
                          {folder}
                        </span>
                      </div>
  ...
  // Portal renders the modal at document.body level, escaping all ancestor stacking contexts
