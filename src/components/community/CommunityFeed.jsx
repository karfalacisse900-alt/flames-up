          ...
          try {
            ...
            if (table === "posts") {
              if (eventType === "DELETE" && record?.id) {
                // Surgically remove deleted post — no full reload
                setPosts(prev => prev.filter(p => String(p.id) !== String(record.id)));
              }
              // Ignore UPDATE events from Supabase RT — upvote state managed locally
            }
