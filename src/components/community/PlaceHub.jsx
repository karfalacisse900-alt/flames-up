...
import { X, MapPin, Bookmark, BookmarkCheck, Image, Video, MessageSquare, Calendar, Lightbulb } from "lucide-react";
...
export default function PlaceHub({ locationName, locationData = {}, user, onClose, onUpvote }) {
  ...
  const videoPosts = posts.filter(p => p.video_url && p.video_url.trim());
  const tipPosts = posts.filter(p => p.place_tags?.length > 0 || (p.body && p.body.length < 200 && !p.image_url && !p.video_url));
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: "var(--bg-app)" }}>
      ...
      <div className="shrink-0" style={{ backgroundColor: "var(--bg-card)", borderBottom: "1px solid var(--border-light)" }}>
        <div className="flex items-center gap-3 px-4 pt-safe pt-4 pb-3">
          ...
          {user && (
            <button onClick={toggleSave} disabled={savedLoading}
              ...
              style={{
                backgroundColor: isSaved ? "var(--accent-primary)" : "transparent",
                borderColor: isSaved ? "var(--accent-primary)" : "var(--border-medium)",
        ...
        {/* Mini map */}
        {hasCoords && (
          <div className="h-32 mx-4 mb-3 rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border-light)" }}>
        ...
        {/* Stats */}
        <div className="flex gap-4 px-5 pb-3 text-xs" style={{ color: "var(--text-hint)" }}>
        ...
        {displayPosts.length === 0 ? (
          <div className="py-16 text-center px-8">
            ...
            <p className="text-sm font-bold" style={{ color: "var(--text-primary)", fontFamily: "var(--font-serif)" }}>
              No {activeTab} yet for {locationName}
            </p>
          ...
          <div className="p-4 space-y-3">
            {displayPosts.map(p => (
              <div key={p.id} className="p-4 rounded-2xl flex gap-3"
                style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border-light)" }}>
                <Lightbulb className="w-4 h-4 mt-0.5 shrink-0" style={{ color: "#D98B62" }} />
                <div>
                  <p className="text-sm leading-relaxed" style={{ color: "var(--text-primary)" }}>
                    {p.body?.replace(/<[^>]*>/g, "").trim()}
                  </p>
                  {p.place_tags?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {p.place_tags.map(tag => (
                  ...
                  <p className="text-[10px] mt-1" style={{ color: "var(--text-hint)" }}>— {p.author_name || "Anonymous"}</p>
                </div>
              </div>
            ))}
          </div>
