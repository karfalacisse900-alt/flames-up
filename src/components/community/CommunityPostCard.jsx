...
import { ChevronDown, Send, Flag, Star } from "lucide-react";

const TYPE_CONFIG = {
  review:          { label: "Review",        emoji: "⭐", color: "#D98B62", bg: "#FFF3E8" },
  ...
  question_of_day: { label: "Q of the Day", emoji: "🌟", color: "#3C6E5A", bg: "#EEF3F0" },
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border-light)", boxShadow: "0 1px 6px rgba(0,0,0,0.04)" }}>
      ...
      <div className="p-4">
        ...
        {post.type === "review" && post.star_rating && (
          <div className="flex gap-0.5 mb-1.5">
            {[1,2,3,4,5].map(n => (
              <Star key={n} className="w-3.5 h-3.5" fill={n <= post.star_rating ? "#D98B62" : "none"}
                style={{ color: n <= post.star_rating ? "#D98B62" : "var(--border-medium)" }} />
            ))}
          </div>
        )}
