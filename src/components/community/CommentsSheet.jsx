  ...
  return (
    <div className="flex gap-3 py-3" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
      {reply.author_avatar_url ? (
        ...
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: `linear-gradient(135deg, ${color}33, ${color}55)`, color }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2 mb-1">
          ...
          {isOwn && <button onClick={() => onDelete(reply)} className="ml-auto p-1 rounded-full" style={{ color: "#E05C7A" }}><Trash2 className="w-3 h-3" /></button>}
        </div>
        {reply.type === "voice" ? <VoicePlayer audioUrl={reply.audio_url} />
          : reply.type === "image" && reply.image_url ? <img src={reply.image_url} alt="comment" className="rounded-xl mt-1 max-w-full object-cover" style={{ maxHeight: 200 }} />
          : reply.type === "sticker" ? <span className="text-4xl">{reply.body}</span>
          : reply.type === "gif" && reply.gif_url ? <img src={reply.gif_url} alt="GIF" className="rounded-xl mt-1 max-w-full" style={{ maxHeight: 180 }} />
          : <p className="text-sm leading-relaxed" style={{ color: "var(--text-primary)" }}><SmartText text={reply.body} /></p>}
        <button onClick={() => setLiked(v => !v)} className="mt-1.5 text-xs flex items-center gap-1" style={{ color: liked ? "#E05C7A" : "var(--text-hint)" }}>
          {liked ? "❤️" : "🤍"} Like
        </button>
      </div>
    </div>
