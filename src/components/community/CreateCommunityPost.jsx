...
import PlaceTagsSelector from "./PlaceTagsSelector";
...
export default function CreateCommunityPost({ user, onClose, onCreated, challengeContext }) {
  ...
  const handleSubmit = async () => {
    ...
    if (imageFiles.length > 0) {
      setUploading(true);
      try {
        const uploads = await Promise.all(imageFiles.map(({ file }) => uploadToR2(file, "posts/images")));
      ...
      setUploading(true);
      try {
        const { file_url } = await uploadToR2(imageFile, "posts/images");
      ...
      setUploading(true);
      try {
        const { file_url } = await uploadToR2(videoFile, "posts/videos");
    ...
    if (!modResult.safe) {
      const newPost = await base44.entities.CommunityPost.create({
        type: effectiveType, body: cleanedBody,
        ...
        place_tags: placeTags.length > 0 ? placeTags : undefined,
