...
import {
  Globe, MapPin, PenSquare, ChevronDown, Search, X
} from "lucide-react";
...
export default function CommunityFeed({ user }) {
  ...
  // GPS detection
  const { coords, locationInfo, permission, detect } = useLocationDetection({
    autoDetect: activeFilter === "nearby",
  });
  ...
  // Trigger GPS when user switches to nearby
  useEffect(() => {
  ...
  const filteredPosts = useCallback(() => {
    ...
    // Location search query (country, city, region, place_name)
    if (searchQuery.trim()) {
    ...
    if (activeFilter === "nearby") {
      ...
      return posts.filter((p) => {
        ...
        const dist = haversineKm(coords.lat, coords.lng, p.location_lat, p.location_lng);
    ...
    if (activeFilter === "country") {
      ...
      return posts.filter(
        (p) =>
          p.location_country?.toLowerCase() === selectedCountry.toLowerCase()
      );
    }
  ...
  // Derive unique countries & cities from posts for dropdowns
  ...
  return (
    <div className="pb-8">
      ...
      <div className="sticky top-0 z-20 px-3 pt-3 pb-0" style={{ backgroundColor: "var(--bg-app)" }}>
        ...
        {/* Row 2: Sub-controls per active filter */}
        {activeFilter === "nearby" && (
            ...
            <div className="px-4 py-12 text-center">
              ...
              <p style={{ color: "var(--text-hint)" }}>
                {searchQuery
                  ...
                  : activeFilter === "nearby"
                  ? "No posts within this radius yet"
