...
// Generates "why use it" bullets from description + tags
function getKeyFeatures(item) {
  const features = [];
  if (item.tags?.length) {
    item.tags.slice(0, 3).forEach(t => features.push(t));
  }
  if (item.platforms?.length) {
    features.push(`Available on ${item.platforms.slice(0, 3).join(", ")}`);
  }
  if (item.pricing) features.push(item.pricing === "Free" ? "100% Free" : item.pricing);
  if (features.length < 3 && item.long_description) {
    // Extract a short phrase from long description
    const words = item.long_description.split(" ").slice(0, 6).join(" ");
    features.push(words + "...");
  }
        ...
        >
          ...
          <div className="overflow-y-auto" style={{ maxHeight: "calc(85vh - 48px)" }}>
            ...
            <div className="mx-4 mb-4 rounded-2xl overflow-hidden relative" style={{ height: 140, background: getGradient(item.category) }}>
              ...
              <div style={{ position: "absolute", bottom: -20, left: 60, width: 100, height: 100, borderRadius: "50%", background: "rgba(0,0,0,0.1)", filter: "blur(20px)" }} />
              ...
              <div className="absolute top-4 right-4 flex flex-col gap-1 items-end">
                ...
                {item.is_sponsored && <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-white/25 text-white">Sponsored</span>}
              </div>
            </div>
              ...
              <div className="flex items-center gap-3 mb-4 flex-wrap">
                {(item.avg_rating || 0) > 0 && (
                  <div className="flex items-center gap-1 px-2.5 py-1 rounded-full" style={{ backgroundColor: "var(--bg-subtle)" }}>
                    ...
                    {item.review_count > 0 && (
                      <span className="text-[10px]" style={{ color: "var(--text-hint)" }}>({item.review_count})</span>
                    )}
                  </div>
                )}
                {item.pricing && (
