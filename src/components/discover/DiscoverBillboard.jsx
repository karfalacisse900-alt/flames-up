    ...
    <div className="px-5 mb-4">
      {isPersonalized && (
        <div className="flex items-center gap-1.5 mb-2 px-1">
          ...
          <span className="text-[10px] font-medium" style={{ color: "var(--text-hint)" }}>Curated for you</span>
        </div>
      )}
      <div
      ...
      >
          ...
          >
            <div className="p-5">
              ...
              <div className="flex items-start gap-4">
                <div className="shrink-0"><DiscoverLogo item={item} size="xl" /></div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm leading-relaxed line-clamp-3" style={{ color: "var(--text-secondary)" }}>{item.description}</p>
                </div>
              </div>
              {item.tags?.length > 0 && (
        ...
        {aiLoading && (
          <div className="absolute bottom-0 left-0 right-0 h-0.5 overflow-hidden" style={{ backgroundColor: "var(--border-light)" }}>
            <motion.div className="h-full" style={{ backgroundColor: "var(--accent-primary)", width: "40%" }}
        ...
        {slides.length > 1 && (
          <div className="flex justify-center gap-1.5 pb-4">
            {slides.map((_, i) => (
              <button key={i} onClick={() => go(i > index ? 1 : -1, i)} className="rounded-full transition-all duration-300"
                style={{ width: i === index ? 18 : 6, height: 6, backgroundColor: i === index ? "var(--accent-primary)" : "var(--border-medium)" }} />
            ))}
          </div>
        )}
      </div>
