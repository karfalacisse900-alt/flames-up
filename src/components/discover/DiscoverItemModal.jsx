...
import {
  X, ExternalLink, ThumbsUp, Send, Globe, Smartphone, Monitor,
  Bookmark, BookmarkCheck, Share2, Twitter, Link2, Check,
  Star, Zap, Users, Crown
} from "lucide-react";
...
export default function DiscoverItemModal({ item, user, onClose, onOpenRelated, allItems = [] }) {
  ...
  const submitReview = useMutation({
    mutationFn: async (data) => {
      ...
      await base44.entities.DiscoverItem.update(item.id, {
  ...
  /*
    FIX — Modal overlap / accidental-close bug:
    1. Changed backdrop from onMouseDown → onClick so the close only fires on a
       full click, not on mousedown (which was triggering during scroll/drag on mobile).
    2. Wrapped everything in createPortal() so the modal renders at document.body
       level, completely outside the Discover feed's DOM. This prevents any ancestor
       overflow, transform, or z-index from clipping or hiding the modal.
    3. Added `isolation: "isolate"` on the backdrop to create a proper stacking
       context that sits above all feed content.
  */
  const modalContent = (
    <div
      ...
      onClick={onClose}  // FIX: was onMouseDown — changed to onClick
    >
      <div
      ...
      >
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map(s => (
                    <Star
                      key={s}
                      className={`w-3.5 h-3.5 ${s <= Math.round(item.avg_rating || 0)
                ...
                {item.review_count > 0 && (
                  <span className="text-white/60 text-xs">({item.review_count})</span>
                )}
                {item.pricing && (
                  <span
        ...
        {showShare && (
          <div
            className="shrink-0 px-5 py-3 flex gap-2 flex-wrap"
            style={{ backgroundColor: "#F0F4F8", borderBottom: "1px solid #E2E8F0" }}
          >
            <a
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          <div className="px-5 pt-4 pb-8 space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {item.tags?.map(tag => (
                <span key={tag}
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${cat.from}20, ${cat.to}20)`,
                    color: cat.from,
                    border: `1px solid ${cat.from}30`,
                  }}>
                  #{tag}
                </span>
              ))}
            ...
            <div className="rounded-2xl p-4"
              style={{ background: `linear-gradient(135deg, ${cat.from}08, ${cat.to}08)`, border: `1px solid ${cat.from}20` }}>
  ...
  // Portal renders modal at document.body, escaping all ancestor stacking contexts
