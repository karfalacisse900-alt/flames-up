...
const BOROUGHS = [
  {
    ...
    neighborhoods: [
      ...
      { name: "Chinatown & Little Italy", description: "Authentic dim sum, fish markets, festival streets, and generations of immigrant community life", image: "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&h=300&fit=crop", spots: [{ name: "Columbus Park", type: "🌿 Park", cost: "Free", desc: "Watch mahjong games, tai chi, and everyday community life. One of the most authentic free experiences in NYC." }, { name: "Canal Street Market", type: "🛍️ Market", cost: "Free entry", desc: "Rotating local vendors with food, art, plants, and unique finds. Great place to spend an hour." }, { name: "Museum of Chinese in America", type: "🏛️ Museum", cost: "$14", desc: "Powerful, thoughtful museum telling the stories of Chinese Americans in their own words." }, { name: "Mulberry Street", type: "🍕 Dining", cost: "Varies", desc: "Walk the original Little Italy strip — try a cannoli from Ferrara Cafe (since 1892). Free to walk." }, { name: "Chinatown Ice Cream Factory", type: "🍦 Food", cost: "$4–7", desc: "Legendary shop with wild flavors like taro, lychee, and red bean. Always a line, always worth it." }, { name: "Canal Street Shops", type: "🛍️ Shopping", cost: "Free to browse", desc: "Bargain everything — bags, watches, souvenirs, fabrics. A chaotic, fun NYC experience." }, { name: "Mosco Street Garden", type: "🌿 Garden", cost: "Free", desc: "A tiny hidden community garden tucked between buildings — you'll feel like you discovered a secret." }, { name: "Nom Wah Tea Parlor", type: "🍽️ Dining", cost: "$10–20", desc: "NYC's oldest dim sum restaurant (1920). Go for the casual weekday lunch experience — no wait." }] },
    ],
  },
  {
...
const QUICK_SUGGESTIONS = [
  ...
  "Cheap date ideas in Manhattan",
  "Free things to do this weekend",
  "Hidden gems most tourists miss",
  "Best food spots in Queens",
  "Things to do at night in NYC",
];

function AIChatModal({ onClose }) {
  const [messages, setMessages] = useState([
    { role: "assistant", text: "Hey! I'm your NYC guide 🗽 Ask me anything — where to eat, what to do, how to plan your day, hidden spots, budget tips, you name it!" }
  ]);
  ...
  const send = async (text) => {
    ...
    const history = newMessages.slice(-6).map(m => `${m.role === "user" ? "User" : "Assistant"}: ${m.text}`).join("\n");

    try {
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `You are a knowledgeable, friendly NYC city guide. Be concise, specific, and helpful. Only recommend real places in New York City. Give practical advice including costs, best times, and neighborhoods. Keep responses to 3-5 sentences max unless asking for a detailed itinerary.
...
Respond naturally as a helpful NYC local guide.`,
      });
  ...
  return (
    <motion.div
      ...
      <div className="shrink-0 px-4 pt-5 pb-4 flex items-center gap-3"
        style={{ paddingTop: "max(env(safe-area-inset-top, 20px), 20px)", borderBottom: "1px solid var(--border-light)", backgroundColor: "var(--bg-nav)", backdropFilter: "blur(20px)" }}>
        ...
        <div className="flex items-center gap-2.5 flex-1">
          ...
          <div>
            <p className="font-bold text-sm leading-tight" style={{ color: "var(--text-primary)" }}>NYC AI Guide</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            {msg.role === "assistant" && (
              <div className="w-7 h-7 rounded-xl flex items-center justify-center text-sm mr-2 mt-0.5 shrink-0"
                style={{ background: "linear-gradient(135deg, #4F46E5, #7C3AED)" }}>
                🗽
              </div>
            )}
            <div className="max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed"
              style={{
                backgroundColor: msg.role === "user" ? "#4F46E5" : "var(--bg-card)",
            ...
            <div className="px-4 py-3 rounded-2xl flex items-center gap-1.5"
              ...
              {[0, 1, 2].map(i => (
                <div key={i} className="w-2 h-2 rounded-full"
                  style={{ backgroundColor: "var(--text-hint)", animation: `bounce 1.2s ${i * 0.2}s infinite` }} />
              ))}
            </div>
          </div>
        )}
      ...
      {/* Quick suggestions (only if just started) */}
      {messages.length <= 1 && (
      ...
      {/* Input */}
      <div className="shrink-0 px-4 pb-6 pt-2" style={{ paddingBottom: "max(env(safe-area-inset-bottom, 24px), 24px)", borderTop: "1px solid var(--border-light)" }}>
        <div className="flex gap-2 items-center">
          <input
            ...
            style={{ backgroundColor: "var(--bg-card)", color: "var(--text-primary)", border: "1px solid var(--border-light)", fontSize: 14, minHeight: "unset", boxShadow: "none" }}
          />
          <button onClick={() => send()} disabled={!input.trim() || loading}
            className="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0"
            style={{ background: "linear-gradient(135deg, #4F46E5, #7C3AED)", opacity: (!input.trim() || loading) ? 0.5 : 1 }}>
  ...
  if (selectedBorough) {
    return (
      <>
        ...
        {/* AI button still accessible from borough detail */}
        <button
          onClick={() => setShowAI(true)}
          className="fixed bottom-24 right-4 z-40 flex items-center gap-2 px-4 py-3 rounded-2xl font-bold text-sm text-white"
        ...
        <AnimatePresence>{showAI && <AIChatModal onClose={() => setShowAI(false)} />}</AnimatePresence>
      </>
    );
  }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--bg-app)" }}>
      ...
      <div className="sticky top-0 z-30 px-4 pb-4"
        style={{ paddingTop: "max(env(safe-area-inset-top, 16px), 16px)", backgroundColor: "var(--bg-nav)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--border-subtle)" }}>
