  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-end justify-center"
      ...
      <motion.div
        ...
        style={{ height: "92dvh", backgroundColor: "#111" }}>
        ...
        <div className="relative shrink-0 bg-black rounded-t-3xl overflow-hidden">
          ...
          <img src={artwork.image_url} alt={artwork.title}
            ...
            style={{ maxHeight: 280, display: imgLoaded ? "block" : "none" }}
            onLoad={() => setImgLoaded(true)} />
          <button onClick={onClose} className="absolute top-3 right-3 bg-black/60 rounded-full p-2">
        ...
        {/* Meta */}
        <div className="px-5 py-3 shrink-0 flex gap-3 justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-bold text-white truncate" style={{ fontFamily: "var(--font-serif)" }}>{artwork.title}</h2>
            <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>by {artwork.user_name} · {timeAgo(artwork.created_date)} ago</p>
            {artwork.description && <p className="text-sm mt-1 leading-relaxed" style={{ color: "rgba(255,255,255,0.65)" }}>{artwork.description}</p>}
          </div>
...
function ArtCard({ art, user, onSelect, onLike }) {
  ...
  return (
    <div className="break-inside-avoid mb-3 rounded-2xl overflow-hidden cursor-pointer group"
      style={{ backgroundColor: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)" }}
...
// ── Upload Modal (with photo editor integration) ──────────────────────────────
...
export default function Gallery() {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ backgroundColor: "var(--bg-app)", overflowX: "hidden", maxWidth: "100%", width: "100%" }}>

      {/* ── Header ── */}
      <div className="px-4 pt-5 pb-0 sticky top-0 z-20" style={{ backgroundColor: "var(--bg-app)" }}>
        ...
        <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-3">
          {TABS.map(t => {
            ...
            return (
              t.id === "challenges"
                ? <Link key={t.id} to={createPageUrl("WeeklyChallenges")}
                ...
                : t.id === "fame"
                  ? <Link key={t.id} to={createPageUrl("HallOfFame")}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-2xl text-xs font-semibold whitespace-nowrap shrink-0 transition-all"
                      style={{ backgroundColor: "var(--bg-card)", color: "var(--text-secondary)", border: "1px solid var(--border-light)" }}>
                      <Icon className="w-3.5 h-3.5" /> {t.label}
                    </Link>
            ...
            <div className="flex gap-1.5 overflow-x-auto scrollbar-hide pb-3">
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setCategory(cat)}
                  className="px-3 py-1 rounded-full text-xs capitalize whitespace-nowrap shrink-0 transition-all"
                  style={{
        ...
        {/* Bottom border */}
        <div style={{ height: 1, backgroundColor: "var(--border-light)", marginLeft: -16, marginRight: -16 }} />
      </div>
        ...
        <>
          ...
          {isLoading ? (
            ...
            <div className="px-4 pt-4"
              style={{ columns: "2", columnGap: "12px", maxWidth: "100%", overflowX: "hidden", width: "100%" }}>
              {sorted.map(art => (
