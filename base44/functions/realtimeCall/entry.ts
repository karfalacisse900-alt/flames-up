...
function getAuthHeader() {
  ...
  // If API key is already a full Authorization header value (e.g. "Basic xxxx" copied from dashboard)
...
async function createMeeting(title) {
  ...
  console.log("createMeeting response:", JSON.stringify(data));
...
async function addParticipant(meetingId, name, presetName) {
  const body = { name, client_specific_id: name };
